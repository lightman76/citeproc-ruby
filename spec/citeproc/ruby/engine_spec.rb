  ...
  describe 'The CiteProc-Ruby Engine' do
    ...
    describe '#bibliography' do
      ...
      describe 'should order entries correctly' do
        before(:each) do
          cp << items(:aaron2).data
          cp << items(:aaron1).data
          cp << items(:abbott).data
        end

        it 'should render all items in the correct order when run twice' do
          #I generate HTML and text versions of the bibliography to return in an API call and the 2nd format wasn't
          # being properly ordered
          cp_bib1_hash = cp.bibliography.to_citeproc
          cp_bib2_hash = cp.bibliography.to_citeproc

          bib_entries = cp_bib1_hash[1]
          expect(bib_entries[0]).to start_with('Aaron, H. Baseball')
          expect(bib_entries[1]).to start_with('Aaron, H. Spitball')
          expect(bib_entries[2]).to start_with('Abbott and Costello')
          bib_entries = cp_bib2_hash[1]
          expect(bib_entries[0]).to start_with('Aaron, H. Baseball')
          expect(bib_entries[1]).to start_with('Aaron, H. Spitball')
          expect(bib_entries[2]).to start_with('Abbott and Costello')
        end
